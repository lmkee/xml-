...
origin_dir = './JPEGImages/Image/'
target_dir1 = './spim/'
annota_dir = './Annotations/anno/'
target_dir2 = './spannota/'
def clip_img(No, oriname):
    ...
    i = 0
    win_size = 600#分块的大小
    stride = 450#重叠的大小，设置这个可以使分块有重叠 350 （值越大分割越少，非重叠？）
    for r in range(0, h - win_size, stride):
        for c in range(0, w - win_size, stride):
            flag = np.zeros([1,15]) #缺陷数目增多时，调大
            youwu = False
            xiefou = True
